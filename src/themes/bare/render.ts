...
/**
 * Plain machine markup and behaviour with no styling of its own. The bare theme
 * uses it as-is; new themes can start from it while their own markup is built.
 */
export function renderBasic(root: HTMLElement, machine: Machine): () => void {
    const cleanup: (() => void)[] = []
    root.innerHTML = `
      <main class="machine">
        <div class="reels">
